...
int main()
{
	...
	path.resize(3);
	dist.resize(3);
	digraph2.addWeightedEdge(0, 1, 3);
	digraph2.addWeightedEdge(0, 2, 4);
	digraph2.addWeightedEdge(2, 1, -2);
	cout << "Bellman-Ford: " << endl;
	digraph2.print();
	digraph2.dijkstra(start, path, dist);
	target = 1;
	start = 0;
	for (int v = target; v != -1; v = path[v])
	{
	...
	digraph2.bellmanFord(start, path, dist);
