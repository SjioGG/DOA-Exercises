...
vector<int> Graph::topologicalSortModified(int startVertex)
{
	stack<int> sorting;
	vector<int> path(adj.size());
	vector<bool> visited(adj.size(), false);
	dfs(startVertex, visited, path, sorting);

	vector<int> topOrder;
	while (!sorting.empty())
	{
		topOrder.push_back(sorting.top());
		sorting.pop();
	}
...
void Graph::dijkstraOnDAG(int source, vector<int> &path, vector<int> &dist, const vector<int> &topOrder)
{
	vector<bool> visited(adj.size());
	for (int v = 0; v < adj.size(); ++v)
	{
	...
	{
		int u = topOrder[i]; // Get the next vertex, in topological order according to the dfs based one that was provided.
		visited[u] = true;
		for (auto v : adj[u])
		{
...
void Graph::dijkstra_nopq(int s, vector<int> &path, vector<int> &dist)
{
	...
	for (int c = 0; c < adj.size(); c++)
	{
		...
		for (auto v : adj[u])
		{
			if (!visited[v] && weight[u][v] && dist[u] != INFINITY && dist[u] + weight[u][v] < dist[v])
			{
				dist[v] = dist[u] + weight[u][v];
				path[v] = u;
			}
