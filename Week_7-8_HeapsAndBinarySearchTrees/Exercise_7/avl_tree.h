...
public:
	...
	bool ltAlgorithm() const // linear-time algorithm
	{
		return ltAlgorithm(root);
	}
...
private:
	...
	bool ltAlgorithm(AvlNode *node) const // linear-time algorithm
	{
		...
		if (abs(leftHeight - rightHeight) > 1) // if the difference between the heights of the left and right subtrees is greater than 1 return false
		{
			return false;
		}
		else if (!ltAlgorithm(node->left) || !ltAlgorithm(node->right)) // if the left or right subtree is not an AVL tree return false
		{
			return false;
		}
		else
		{
			return true; // if the tree is an AVL tree return true
		}
	}
};
